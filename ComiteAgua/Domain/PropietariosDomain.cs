            ...
            try
            {
                ...
                agrid.SetData(
                        ...
                        select new
                        {
                            ...
                            ConceptoPagoId = (e.Toma.Select(c => c.Convenio.Select(co => co.EstatusConvenioId).LastOrDefault()).LastOrDefault() == (int)EstatusConvenioDomain.EstatusConvenioEnum.Activo ||
                                            e.Toma.Select(c => c.Convenio.Select(co => co.EstatusConvenioId).LastOrDefault()).LastOrDefault() == (int)EstatusConvenioDomain.EstatusConvenioEnum.Cancelado) &&
                                            e.Toma.Select(c => c.Convenio.Select(co => co.Eliminado).LastOrDefault()).LastOrDefault() == null ? Convert.ToInt32(ConceptosPagoDomain.ConceptosPagoDomainEnum.Convenio) :
